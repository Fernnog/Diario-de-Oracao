    ...
    if (docSnap.exists()) {
        ...
        if (perseveranceData.lastInteractionDate) {
            ...
            if (lastInteractionDate.getTime() < today.getTime()) {
                perseveranceData.consecutiveDays = 0; // Reset if last interaction was not yesterday or today
...
function updatePerseveranceUI() {
    ...
    const percentage = Math.min((perseveranceData.consecutiveDays / 100) * 100, 100); // Cap at 100% for UI
